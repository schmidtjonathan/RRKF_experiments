...
function evaluation_for_spatial_lengthscale(ℓₓ)
    ...
    kf_trial = @benchmark RRKF.estimate_states(
        $RRKF.SqrtKalmanFilter(),
        $ssm,
        $simulation_grid,
        $train_observations,
    ...
    for cur_nval in nval_list
        ...
        cur_rrkf_trial = @benchmark RRKF.estimate_states(
            RRKF.RankReducedKalmanFilter($cur_nval, 1),
            $ssm,
            $simulation_grid,
            $train_observations,
        ...
        cur_enkf_trial = @benchmark RRKF.estimate_states(
            RRKF.EnsembleKalmanFilter($cur_nval, RRKF.enkf_correct),
            $ssm,
            $simulation_grid,
            $train_observations,
        ...
        push!(enkf_rmse_to_test_per_nval, cur_enkf_rmse_to_test)
        ...
        cur_etkf_trial = @benchmark RRKF.estimate_states(
            RRKF.EnsembleKalmanFilter($cur_nval, RRKF.etkf_correct),
            $ssm,
            $simulation_grid,
            $train_observations,
        ...
        push!(etkf_rmse_to_test_per_nval, cur_etkf_rmse_to_test)
...
save(
    "./out/on-model_err_runtime_matern12.jld2",
    Dict(
